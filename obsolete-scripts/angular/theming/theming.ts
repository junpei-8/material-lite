...
import { Falsy, MlDocument, styling, noop } from '@material-lite/angular-cdk/utils';
...
const shadowTheming: ShadowTheming = {
  ...
  _init(themeBases: ThemeBases): void {
    ...
    while (storageLen < forLen) {
      ...
      const rootSelector = base.rootSelector === void 0 ? 'body' : base.rootSelector;
      if (wrapperClass) {
        const wc = '.' + wrapperClass; // { {
        _entryStyle =
          (rootSelector ? `${rootSelector + wc}{background-color:${theme.background};color:${theme.text}}` : '') +
          wc + ' ' + _entryStyle.replace(/\}\s*\./g, `}${wc} .`).replace(/\,\s*\.(?![0-9])/g, `,${wc} .`);

      } else if (rootSelector) {
        _entryStyle += `${rootSelector}{background-color:${theme.background};color:${theme.text}}`;
      }
